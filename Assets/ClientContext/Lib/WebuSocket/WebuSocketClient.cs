		...
		*/
		...
		private Socket WebSocketHandshake (string urlSource, Dictionary<string, string> additionalHeaderParams, Action<string, Exception> OnError=null) {
			...
			*/
			...
			var requestDataBytesAndHostAndPort = GenerateRequestData(urlSource, additionalHeaderParams);
			
			try {
				sock.Connect(requestDataBytesAndHostAndPort.host, requestDataBytesAndHostAndPort.port);
			} catch (Exception e) {
				ForceCloseSock();
				if (OnError != null) OnError("failed to connect to host:" + requestDataBytesAndHostAndPort.host + " error:" + e, e);
				return null;
			}
			...
			try {
				var result = sock.Send(requestDataBytesAndHostAndPort.requestDataBytes);
		...
		public static RequestDataBytesAndHostAndPort GenerateRequestData (string urlSource, Dictionary<string, string> additionalHeaderParams) {
